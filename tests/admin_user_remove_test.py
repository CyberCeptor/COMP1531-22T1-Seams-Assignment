...
def test_admin_user_remove_works(clear_and_register):
    ...
    id1 = clear_and_register[0]['token']

    token2 = clear_and_register[1]['auth_user_id']
    ...
    resp5 = requests.get(config.url + 'users/all/v1', params={'token': token1})
    assert resp5.status_code == 200
    users = resp5.json()
    assert len(users['users']) == 1

    # there will only be one channel member left and no channel owners
    resp6 = requests.get(config.url + 'channel/details/v2',
                         params={'token': token1, 'channel_id': chan_id})
    assert resp6.status_code == 200
    chan_data = resp6.json()
    assert len(chan_data['owner_members']) == 0
    assert len(chan_data['all_members']) == 0
    ...
    resp8 = requests.get(config.url + 'dm/details/v1',
    ...
    # reusable
    resp11 = requests.post(config.url + 'auth/register/v2', 
                         json={'email': 'abc@def.com', 'password': 'password',
                               'name_first': 'first', 'name_last': 'last'})
    assert resp11.status_code == 200
    user2_new = resp11.json()
    id2_new = user2_new['auth_user_id']
    assert id2 != id2_new

    resp12 = requests.get(config.url + 'user/profile/v1', 
