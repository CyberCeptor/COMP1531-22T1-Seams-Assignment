...
def test_channel_removeowner_working(clear_register_two_createchannel_twoowners):
    ...
    assert user1_id in [k['u_id'] for k in chan_json['all_members']]
    assert 'abc@def.com' in [k['email'] for k in chan_json['all_members']]
    assert 'first' in [k['name_first'] for k in chan_json['all_members']]
    assert 'last' in [k['name_last'] for k in chan_json['all_members']]
    assert 'firstlast' in [k['handle_str'] for k in chan_json['all_members']]
