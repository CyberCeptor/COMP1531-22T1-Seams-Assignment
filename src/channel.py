...
from src.other import check_user_is_member, check_valid_channel_id, return_channel_data, return_member_data
...
def channel_invite_v1(auth_user_id, channel_id, u_id):
    ...
    check_user_is_member(auth_user_id, channel_id)
    
    if check_user_is_member(u_id, channel_id):
...
def channel_join_v1(auth_user_id, channel_id):
    ...
    if check_user_is_member(auth_user_id, channel_id):
        raise InputError('Invitee is already in the channel')
...
def channel_leave_v1(token, channel_id):
    ...
    data_store.get()
    check_valid_channel_id(channel_id)
    channel_data = return_channel_data(channel_id)

    token_valid_check(token)
    user_id = token_get_user_id(token)
    user_data = return_member_data(user_id, channel_id)
